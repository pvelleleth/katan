<script lang="ts">
	import { resourceKeys, type ResourceKey } from './trade';

	export let gameState: any;
	export let playerId: string;
	export let players: any[];
	export let onAccept: (tradeId: number) => void;
	export let onReject: (tradeId: number) => void;
	export let onCancel: (tradeId: number) => void;
	export let onFinalize: (tradeId: number, partnerPlayerId: string) => void;
	...
	$: pendingTrades = (gameState?.turn?.pending_player_trades ?? []) as any[];
	...
	function canCoverRequested(trade: any) {
		return resourceKeys.every((resource) => (hand[resource] ?? 0) >= (trade?.requested?.[resource] ?? 0));
	}
</script>

{#if pendingTrades.length > 0}
	<div class="absolute top-4 right-4 z-20 flex w-full max-w-[280px] flex-col gap-2">
		{#each pendingTrades as pendingTrade (pendingTrade.id)}
			{@const isProposer = pendingTrade.player_id === playerId}
			{@const responses = pendingTrade.responses ?? []}
			{@const viewerResponse = pendingTrade.viewer_response ?? null}
			{@const canRespond = !!pendingTrade.can_respond && !isProposer}
			{@const canFinalize = !!pendingTrade.can_finalize && isProposer}
			{@const canCover = canCoverRequested(pendingTrade)}
			<div
				class="rounded-2xl border border-wood/10 bg-white/95 p-3 shadow-xl backdrop-blur-md"
			>
				<div class="flex flex-col gap-2.5">
					...
					{#if isProposer}
								...
								{#each responses as response}
									...
									<div class="flex items-center justify-between rounded-lg bg-white/50 px-2 py-1">
										...
										{#if response.status === 'Accepted'}
											<button
												on:click={() => onFinalize(pendingTrade.id, response.player_id)}
						...
						<div class="flex justify-end">
							<button
								on:click={() => onCancel(pendingTrade.id)}
					...
					{:else}
						<div class="flex items-center justify-between gap-2 pt-1">
							{#if viewerResponse === 'Accepted'}
								...
								<button
									on:click={() => onReject(pendingTrade.id)}
							...
							{:else if viewerResponse === 'Rejected'}
								...
								<button
									on:click={() => onAccept(pendingTrade.id)}
									disabled={!canRespond || !canCover}
